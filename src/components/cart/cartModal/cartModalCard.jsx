...
import useCartProducts from './CartModalCardLogic';

function CartModalCard() {
    const { state } = useContext(cartContext);
    const { cartProducts, error } = useCartProducts(state.cart);
        ...
        <div className='flex flex-col justify-center items-center'>
            ...
            {cartProducts.length === 0 ? (
                ...
                cartProducts.map((product, index) => (
                    <div key={index} className='w-full rounded-md px-4 py-2 m-auto flex justify-between items-center'>
